...
def path_date(path: Path) -> str | None:
    for part in path.parts:
        if DATE_PATTERN.match(part): return part
    return None
...
def matches_target_date(*, path: Path, payload: dict, target_date: str) -> tuple[bool, str | None]:
    ...
    if collection_date and str(collection_date) == target_date: return True, "collection_run_date"
    file_date = path_date(path)
    if file_date == target_date: return True, "path_date"
    if published_date(payload) == target_date: return True, "published_at"
    return False, None

def article_identity(*, path: Path, payload: dict, source_folder: str) -> str:
    if payload.get("canonical_url"): return f"{source_folder}|{payload['canonical_url']}"
    if payload.get("source_url"): return f"{source_folder}|{payload['source_url']}"
    if path.parent.name: return f"{source_folder}|{path.parent.name}"
...
def find_latest_article_files(*, target_date: str) -> tuple[list[Path], dict]:
    ...
    source_counts = {}
    match_methods = {"collection_run_date": 0, "path_date": 0, "published_at": 0}
    for source_folder in ACTIVE_SOURCE_FOLDERS:
        ...
        for path in source_root.rglob("*.json"):
            scanned_files += 1; source_counts[source_folder]["scanned"] += 1
            try: payload = load_json(path)
            except Exception: load_failures += 1; continue
            if not isinstance(payload, dict): continue
            valid_json_files += 1
            matched, method = matches_target_date(path=path, payload=payload, target_date=target_date)
            if not matched: continue
            matched_files += 1; source_counts[source_folder]["matched"] += 1
            if method: match_methods[method] += 1
            ...
            if existing is None or candidate_version > existing["version"] or (candidate_version == existing["version"] and mtime > existing["mtime"]):
                latest_by_article[identity] = {"path": path, "version": candidate_version, "mtime": mtime}
...
def find_available_dates() -> dict:
    ...
    for source_folder in ACTIVE_SOURCE_FOLDERS:
        source_root = STAGED_ROOT / source_folder
        source_dates = {}
        if not source_root.exists(): dates[source_folder] = {}; continue
        for path in source_root.rglob("*.json"):
            try: payload = load_json(path)
            except Exception: continue
            candidates = set()
            collection_date = payload.get("collection_run_date")
            if collection_date and DATE_PATTERN.match(str(collection_date)): candidates.add(str(collection_date))
            file_value = path_date(path)
            if file_value: candidates.add(file_value)
            pub_date = published_date(payload)
            if pub_date: candidates.add(pub_date)
            for value in candidates: source_dates[value] = source_dates.get(value, 0) + 1
...
def article_id_from_path(path: Path, article: dict) -> str:
    if path.parent and path.parent.name and path.parent.name not in {"daily_ft", "ceylon_today"} and not DATE_PATTERN.match(path.parent.name):
        return path.parent.name
    if article.get("content_hash"): return str(article["content_hash"])[:16]
...
def save_analysis(*, run_date: str, input_path: Path, article: dict, analysis) -> Path:
    ...
    try: input_file_value = str(input_path.relative_to(PROJECT_ROOT))
    except ValueError: input_file_value = str(input_path)
    payload = {
...
def main() -> None:
    ...
    parser.add_argument("--date", help="Target date YYYY-MM-DD. Matches collection date, staged path date, or publication date.")
    ...
    print(f"BANK SUPERVISION NEWS INTELLIGENCE — STAGE 1\nTarget date: {run_date}\nRules version: {config.get('version')}\nUnique articles: {diagnostics['unique_articles_selected']}")
    if not article_files:
        print("No staged articles matched the requested date.")
        for source, date_counts in find_available_dates().items(): print(source, date_counts)
        return
    ...
    for path in article_files:
        try:
            ...
            if not headline: raise ValueError("HEADLINE_MISSING")
            if not article_text: raise ValueError("ARTICLE_TEXT_MISSING")
            if not source_url: raise ValueError("SOURCE_URL_MISSING")
            ...
            if analysis.gate_decision == "INCLUDE_CANDIDATE": include_count += 1
            elif analysis.gate_decision == "WATCH": watch_count += 1
            else: exclude_count += 1
            review_rows.append({
                "source_name": article.get("source_name"), "headline": headline, "published_at": article.get("published_at"), "source_url": source_url,
                "financial_score": analysis.financial_relevance_score, "financial_label": analysis.financial_relevance_label,
                "pr_score": analysis.pr_noise_score, "pr_label": analysis.pr_noise_label,
                "article_type_hint": analysis.article_type_hint, "gate_decision": analysis.gate_decision,
                ...
                "human_label": "", "review_notes": "", "analysis_file": str(output_path),
            })
        except Exception as exc:
            failed_count += 1
            print(f"FAILED: {type(exc).__name__}: {exc}")
    fieldnames = ["source_name","headline","published_at","source_url","financial_score","financial_label","pr_score","pr_label","article_type_hint","gate_decision","institutions","matched_financial_terms","matched_pr_terms","pr_counter_signals","human_label","review_notes","analysis_file"]
    with review_path.open("w", newline="", encoding="utf-8-sig") as file:
        writer = csv.DictWriter(file, fieldnames=fieldnames); writer.writeheader(); writer.writerows(review_rows)
    print(f"STAGE-1 INTELLIGENCE COMPLETE\nArticles analysed: {len(review_rows)}\nInclude candidates: {include_count}\nWatch: {watch_count}\nExcluded as noise: {exclude_count}\nFailures: {failed_count}\nHuman review file: {review_path}")
