...
def run_collection_job() -> None:
    ...
    if not COLLECTION_SCRIPT.exists():
        ...
        write_scheduler_status(status="ERROR", last_run_started_at=started_at.isoformat(), last_run_completed_at=completed_at.isoformat(), last_run_result="COLLECTION_SCRIPT_MISSING")
        return
    try:
        ...
        if result.returncode == 0:
            ...
            write_scheduler_status(status="RUNNING", last_run_started_at=started_at.isoformat(), last_run_completed_at=completed_at.isoformat(), last_run_result="SUCCESS", last_return_code=result.returncode)
        else:
            LOGGER.error("Master collection returned non-zero code: %s", result.returncode)
            write_scheduler_status(status="RUNNING", last_run_started_at=started_at.isoformat(), last_run_completed_at=completed_at.isoformat(), last_run_result="FAILED", last_return_code=result.returncode)
    except Exception as exc:
        completed_at = datetime.now(SRI_LANKA_TZ)
        LOGGER.exception("Scheduler could not execute master collection.")
        write_scheduler_status(status="RUNNING", last_run_started_at=started_at.isoformat(), last_run_completed_at=completed_at.isoformat(), last_run_result="EXECUTION_ERROR", error_type=type(exc).__name__, error_message=str(exc))
    LOGGER.info("=" * 70)
...
def print_schedule(scheduler: BlockingScheduler) -> None:
    ...
    for hour in range(8, 18): print(f"    {hour:02d}:00")
    print(f"\nNext run : {next_run.isoformat() if next_run else 'Unable to calculate'}")
    print("\nPress Ctrl+C to stop scheduler.")
    print("=" * 72 + "\n")

def main() -> None:
    parser = argparse.ArgumentParser(description="Bank Supervision News Collection Scheduler")
    parser.add_argument("--run-now", action="store_true", help="Run collection once immediately before entering the normal schedule.")
    args = parser.parse_args()
    if args.run_now:
        LOGGER.info("Immediate run requested.")
        run_collection_job()
    scheduler = BlockingScheduler(timezone=SRI_LANKA_TZ, job_defaults={"max_instances": 1, "coalesce": True, "misfire_grace_time": 1800})
    trigger = CronTrigger(day_of_week="mon-fri", hour=SCHEDULE_HOURS, minute=SCHEDULE_MINUTE, second=0, timezone=SRI_LANKA_TZ)
    scheduler.add_job(run_collection_job, trigger=trigger, id="bank_news_collection", name="Bank Supervision News Collection", replace_existing=True)
    scheduler.add_listener(scheduler_event_listener, EVENT_JOB_EXECUTED | EVENT_JOB_ERROR | EVENT_JOB_MISSED)
    next_run = get_next_run_time(scheduler)
    write_scheduler_status(status="RUNNING", scheduler_started_at=datetime.now(SRI_LANKA_TZ).isoformat(), next_run=next_run.isoformat() if next_run else None)
    ...
    try:
        ...
        try:
            write_scheduler_status(status="STOPPED", scheduler_stopped_at=datetime.now(SRI_LANKA_TZ).isoformat())
        except Exception:
            pass
